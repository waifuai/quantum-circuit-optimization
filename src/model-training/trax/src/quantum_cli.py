...
def create_data_pipeline(input_filepath: str, output_filepath: str, batch_size: int) -> Tuple[trax.data.Serial, int]:
    """Creates data pipeline and calculates vocab size."""
    ...
    vocab = quantum_circuit_data_generator(input_filepath, output_filepath)
...
def get_model(vocab_size: int, mode: str = 'train'):
...
def predict(model_dir: str, input_circuit: str, vocab_size:int) -> str:
    """Predicts the optimized circuit."""
    model = get_model(vocab_size, mode='predict')
    model_file = os.path.join(model_dir, "model.pkl.gz")
    model.init_from_file(model_file, weights_only=True)
    tokenized_input = tokenize_input(input_circuit)
...
def train_model(input_file: str, output_file: str, model_dir: str,
                batch_size: int, n_steps: int, vocab_size: int):
    ...
    model = get_model(vocab_size, mode='train')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Quantum Circuit Optimization CLI")
    ...
    if args.command == "prep":
        ...
        input_file = os.path.join(args.data_dir, "input.txt") # dummy file needed for vocab
        output_file = os.path.join(args.data_dir, "output.txt") # dummy file needed for vocab
        ...
        if not os.path.exists(args.model_dir):
            ...
            sys.exit(1)
        optimized_circuit = predict(args.model_dir, args.input_circuit, vocab_size)
